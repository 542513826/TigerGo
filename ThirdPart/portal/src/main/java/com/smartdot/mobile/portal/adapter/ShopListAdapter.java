...
public class ShopListAdapter extends CommonAdapter<AppDetailBean> {
    ...
    public ShopListAdapter(Context context, List<AppDetailBean> items, int itemLayoutResId) {
    ...
    public void convert(CommonViewHolder viewHolder, AppDetailBean bean) {
        ...
        // 只有第一个和最后一个才会显示对应的豁口图片
        if (position == 0) {
            ((TextView) viewHolder.getView(R.id.tv_bookmark_header)).setVisibility(View.VISIBLE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_line)).setVisibility(View.VISIBLE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_footer)).setVisibility(View.GONE);
        } else {
            ((TextView) viewHolder.getView(R.id.tv_bookmark_line)).setVisibility(View.VISIBLE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_footer)).setVisibility(View.GONE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_header)).setVisibility(View.GONE);
            ((TextView) viewHolder.getView(R.id.tv_line_header)).setVisibility(View.GONE);
        }
        if (position - 1 == items.size()) {
            ((TextView) viewHolder.getView(R.id.tv_bookmark_footer)).setVisibility(View.VISIBLE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_header)).setVisibility(View.GONE);
            ((TextView) viewHolder.getView(R.id.tv_bookmark_line)).setVisibility(View.GONE);
            ((TextView) viewHolder.getView(R.id.tv_line_header)).setVisibility(View.GONE);
        }
